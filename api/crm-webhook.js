...
export default async function handler(req, res) {
  ...
  // Meta Conversion Leads spec: user_data.lead_id deve ser o Meta-generated
  // 15-17 digit leadgen_id. contact.id do Chatwoot (5-7 dígitos) NÃO serve
  // como matching key.
  //
  // FIX 20/04/2026 (Conversion Leads wizard ativação): IceLaser AGORA recebe
  // Lead Ads nativos via webhook page.leadgen → /api/whatsapp cria contato no
  // Chatwoot com identifier="leadgen_{leadgen_id}". Aqui extraímos esse id
  // real Meta-generated e populamos user_data.lead_id — sinal fundamental pro
  // algoritmo CRM matching fechar o ciclo Lead Ad → Purchase/Qualified.
  let leadgenId = null;
  if (contact.identifier && typeof contact.identifier === 'string') {
    const m = contact.identifier.match(/^leadgen_(\d{15,17})$/);
    if (m) {
      leadgenId = m[1];
      userData.lead_id = leadgenId;
    }
  }
  // Fallback: alguns contatos podem ter leadgen_id como custom_attribute
  if (!leadgenId && customAttrs.leadgen_id) {
    const candidate = String(customAttrs.leadgen_id).trim();
