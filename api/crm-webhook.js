...
export default async function handler(req, res) {
  ...
  // Fix CRITICAL 20/04/2026: lead_id handling compliant com Meta Conversion Leads spec.
  ...
  //  3. Custom data `crm_contact_id` mantém referência Chatwoot pra debug no Events Manager
  //
  // IceLaser hoje tem 4 Lead Ads forms ativos (form 1992360114709864, 1613134060085864,
  // 826087098882676) — quando campanhas Lead Ads rodarem, webhook leadgen pode popular
  // leadgen_id no Chatwoot custom_attr pra uso aqui.
  ...
  // Meta spec: 15-17 dígitos numéricos. Validar formato estrito pra evitar accidental IDs errados.
  const leadgenIdValid = leadgenIdRaw && /^\d{15,17}$/.test(String(leadgenIdRaw).trim());
  if (leadgenIdValid) {
    userData.lead_id = String(leadgenIdRaw).trim();
