...
export default async function handler(req, res) {
  ...
  // DESARME por remoção de bia_teste: detecta a TRANSIÇÃO (tinha → não tem mais).
  // Roda para TODO evento (independente de CHATWOOT_BOT_ENABLED). Reusa previousLabels e
  // hasBiaTesteLabel canônicos. Backstop confiável: cron guard em validateFollowupConversation.
  const biaTesteRemoved =
    previousLabels.map((l) => String(l).trim().toLowerCase()).includes('bia_teste') &&
    !hasBiaTesteLabel;
  if (convIdForFU && biaTesteRemoved) {
    try {
      await disarmCascade(convIdForFU, 'bia_teste_removed');
      await clearActiveSession(convIdForFU, 'bia_teste_removed');
      console.log(`[CRM-WEBHOOK] bia_teste removido → cascade desarmado conv=${convIdForFU}`);
    } catch (e) {
      console.error(`[CRM-DISARM-BIATESTE] conv=${convIdForFU} ${e?.message || e}`);
    }
  }
