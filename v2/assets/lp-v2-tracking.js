/**
 * IceLaser LP V2 Conversion — tracking + form wire-up
 *
...
(function () {
  ...
  window.handleLead = function handleLead(event) {
    ...
    try {
      if (window.fbq) {
        fbq('track', 'Lead',
          { content_name: 'Avaliacao Gratuita LP V2', content_category: 'depilacao_laser', value: 0, currency: 'BRL' },
          { eventID: eventId }
        );
        fbq('track', 'CompleteRegistration',
          { content_name: 'Form Submitted V2', content_category: 'depilacao_laser', value: 0, currency: 'BRL' },
          { eventID: eventId + '_cr' }
        );
      }
    } catch (_) {}
    ...
    // === Vercel Analytics ===
    trackEvent('Lead Submit V2', { lp: 'v2-conversion', area_interesse: area, source: origemFB });
    ...
    const obs = new IntersectionObserver(
      (entries) => {
        if (entries[0].isIntersecting && !tracked) {
          tracked = true;
          trackEvent('Scroll ao Formulario V2');
          obs.disconnect();
        }
      },
      { threshold: 0.5 }
  ...
  // [data-countdown]/#countdown-timer (não existem) → ficava '--' eterno.
  // Agora suporta ambos schemas: data-cd="hh"/"mm"/"ss" (Claude Design) +
  // .h/.m/.s legacy + el.textContent inteiro (fallback simples).
  // ════════════════════════════════════════════════════════════════
  (function () {
    ...
    function updateAll() {
      try {
        ...
        timerEls.forEach((el) => {
          // 1. Claude Design schema: <div data-cd="hh|mm|ss">
          ...
          // 2. Legacy schema: split com .h/.m/.s
